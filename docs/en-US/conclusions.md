...
**Supported Hypothesis (V6.5 - Experts as Structural Memory):** In a final extreme hypothesis test, the Router was restricted to an absolute Top-K of 1 (only 1 expert processes the final calculation at a time). By scaling the inactive experts from 10 to 160, the overall accuracy of the model did not stagnate; it increased. While this is not a definitive proof, it strongly suggests that unprocessed experts are not useless redundancy, but function as **passive/structural memory banks**. Their purely topological existence seems to allow the router to create hyper-specific sub-regions to slice the space, pointing to Sparse MoE as a promising Memory Indexing mechanism (*Retrieval System*).

**Absolute Next Step:** Transition from architectural research on synthetic datasets (V1-V6) to practical validation in generative tasks (V8 - Image-to-Code).
