...
**Hipótese Suportada (V6.5 - Experts como Memória Estrutural):** Em um último teste de hipótese extrema, o Roteador foi forçado a usar um Top-K absoluto de 1 (apenas 1 expert processa o cálculo final por vez). Escalando os experts inativos de 10 para 160, a acurácia global do modelo não estagnou; ela subiu. Embora não seja uma prova definitiva, isso sugere fortemente que os experts não processados não são redundância inútil, mas funcionam como **bancos de memória passiva/estrutural**. A sua existência puramente topológica parece permitir ao roteador criar sub-regiões hiperespecíficas para fatiar o espaço, apontando o Sparse MoE como um promissor mecanismo de Indexação de Memória (*Retrieval System*).

**Próximo Passo Absoluto:** Transição de pesquisa arquitetural em datasets sintéticos (V1-V6) para validação prática em tarefas generativas (V8 - Image-to-Code).
