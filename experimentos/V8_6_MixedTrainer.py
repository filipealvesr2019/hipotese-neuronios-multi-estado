...
def main():
    ...
    for expert_id, cats in expert_category_dominance.items():
        ...
        size = expert_sizes[expert_id]
        if total_tokens_won > 0:
            print(f"\n[Expert {expert_id} ({size} neurônios)] Dominou {total_tokens_won} tokens únicos:")
            for cat, count in cats.items():
                if count > 0:
                    pct = (count / total_tokens_won) * 100
                    print(f"  -> {pct:.1f}% pertencem à categoria: {cat}")
        else:
            print(f"\n[Expert {expert_id} ({size} neurônios)] Não dominou estatisticamente nenhuma categoria principal.")
    print("="*50)
