...
def compute_eri(expert_outputs):
    preds = np.argmax(expert_outputs, axis=-1)
    n = preds.shape[1]
...
class V64_RouterCapacity_MoE:
    def __init__(self, input_dim, hidden_sizes, output_dim, router_hidden=64,
        ...
        self.gW1 = rng.randn(input_dim, router_hidden) * 0.1
        self.gb1 = np.zeros(router_hidden)
        self.gW2 = rng.randn(router_hidden, max(8, router_hidden//2)) * 0.1
        self.gb2 = np.zeros(max(8, router_hidden//2))
        self.gW3 = rng.randn(max(8, router_hidden//2), self.n_experts) * 0.1
...
def run_router_capacity_study():
    ...
    router_sizes = [8, 16, 32, 64, 128, 256]
    hidden_sizes = [32]*4 + [64]*4 + [128]*4 + [256]*4 + [512]*4 # Fixando em 20 experts
    ...
    print("\nIniciando Router Capacity Study (Fixado N=20 Experts)...")
    for r_size in router_sizes:
        print(f"\n[Treinando Router Hidden={r_size}]")
        model = V64_RouterCapacity_MoE(input_dim=X.shape[1], hidden_sizes=hidden_sizes, output_dim=10, router_hidden=r_size, top_k=4)
        ...
        for epoch in range(epochs):
            ...
            if epoch == epochs - 1:
                logits_val, gate_val, out_list, _, _, _, gate_probs, _ = model.forward(X_test, temperature=0.7)
                val_acc = np.mean(np.argmax(logits_val, axis=1) == y_test)
                
                usage = np.mean(gate_val, axis=0)
                gini_val = gini_index(usage)
                
                out_list_np = np.stack(out_list, axis=1) # [B, N, Out]
                eri_val = compute_eri(out_list_np)
                
                print(f"  Resultados | Val ACC: {val_acc:.4f} | ERI: {eri_val:.4f} | Gini: {gini_val:.4f} | Tempo total: {time()-t0:.1f}s")
                
                acc_results.append(val_acc)
                gini_results.append(gini_val)
                eri_results.append(eri_val)
    ...
    color = 'tab:blue'
    ax1.set_xlabel('Capacidade do Router (Neurônios Ocultos)')
    ax1.set_ylabel('Accuracy', color=color)
    ax1.plot(router_sizes, acc_results, marker='o', color=color, linewidth=2, label='Accuracy')
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('Gini Index (Pruning)', color=color)
    ax2.plot(router_sizes, gini_results, marker='s', color=color, linewidth=2, linestyle='--', label='Gini Index')
    ax2.tick_params(axis='y', labelcolor=color)
    
    plt.title('Router Capacity Study (Accuracy vs Pruning Gini)')
    fig.tight_layout()
    plt.grid(True)
    plt.savefig('graficos/router_capacity_analysis.png')
    plt.close()
    
    print("\nVisualização salva em graficos/router_capacity_analysis.png")
    
if __name__ == "__main__":
    run_router_capacity_study()
