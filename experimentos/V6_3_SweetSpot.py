...
def run_sweet_spot():
    ...
    for n in n_experts_list:
        ...
        print(f"\n[Treinando N={n} Experts | Top-K=4]")
        model = V63_SweetSpot_MoE(input_dim=X.shape[1], hidden_sizes=hidden_sizes, output_dim=10, top_k=4)
        ...
        for epoch in range(epochs):
            ...
            if epoch == epochs - 1:
                logits_val, gate_val, out_list, _, _, _, _, _ = model.forward(X_test, temperature=0.7)
    ...
    color = 'tab:blue'
    ax1.set_xlabel('Número de Experts (N)')
    ax1.set_ylabel('Accuracy', color=color)
    ax1.plot(n_experts_list, acc_results, marker='o', color=color, linewidth=2, label='Accuracy')
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('Gini Index (Pruning)', color=color)
    ax2.plot(n_experts_list, gini_results, marker='s', color=color, linewidth=2, linestyle='--', label='Gini Index')
    ax2.tick_params(axis='y', labelcolor=color)
    
    plt.title('MoE Sweet Spot Analysis (Accuracy vs Pruning Gini)')
    fig.tight_layout()
    plt.grid(True)
    plt.savefig('graficos/sweet_spot_analysis.png')
    plt.close()
    
    print("\nVisualização salva em graficos/sweet_spot_analysis.png")
