"""
Runner de variantes V4 econômicas para MNIST.

Foco:
- reduzir overhead do gate;
- testar 2 vs 4 especialistas;
- testar skip on/off;
- comparar contra MLP 64/128 já existentes.

Exemplo:
  python experimentos/e1_mnist/codigo/run_mnist_economic_matrix.py --seeds 1 --epochs 5
"""
...
def parse_bool_list(text: str) -> list[bool]:
    ...
    for part in text.split(","):
        ...
        if value in {"1", "true", "yes", "on", "skip"}:
            values.append(True)
        elif value in {"0", "false", "no", "off", "noskip", "no-skip"}:
            values.append(False)
        else:
            raise ValueError(f"Valor booleano invalido: {part}")
    return values
...
def main() -> int:
    ...
    parser.add_argument("--dense-hidden", default="64,128")
    parser.add_argument("--v4-hidden", default="64,128")
    parser.add_argument("--states", default="2,4")
    parser.add_argument("--gate-hidden", default="8,16")
    parser.add_argument("--skip", default="true,false")
    ...
    gate_hidden_list = parse_int_list(args.gate_hidden)
    skip_list = parse_bool_list(args.skip)
    ...
    for seed in seeds:
        for hidden in dense_hidden:
            ...
            if not (args.skip_existing and out_path.exists()):
                cmd = [
                    ...
                    "--lr", str(args.lr),
                    "--l2", str(args.l2),
                    "--out", f"e1_mnist_economic/{out_name}",
                ]
                if args.train_limit:
        ...
        for hidden in v4_hidden:
            for states in states_list:
                for gate_hidden in gate_hidden_list:
                    for use_skip in skip_list:
                        suffix = "skip" if use_skip else "noskip"
                        out_name = f"v4_h{hidden}_s{states}_g{gate_hidden}_{suffix}_seed{seed}.json"
                        out_path = RESULT_DIR / out_name
                        if not (args.skip_existing and out_path.exists()):
                            cmd = [
                                sys.executable, str(EXPERIMENT),
    ...
    for row in summary:
        print(
            f"{row['model']:>11} h={row['hidden']:>3} s={row['states']:>2} "
            f"g={row['gate_hidden']:>2} skip={str(row['use_skip']):>5} "
