...
# =========================
# V6.0 Stable MoE: Competitive Experts + Credit Assignment + MI Regularization
# =========================
class V60MoE:
    def __init__(self, input_dim, hidden, output_dim, n_experts=5, topk=2,
                 lr=0.01, seed=42, temperature=1.5, mi_lambda=0.1):
    ...
    def train_step(self, x, y):
        ...
        for i in range(self.n_experts):
            ...
            # Mutual Information Regularizer (simplified)
            usage = np.mean(gate, axis=0)
            dW2 += self.mi_lambda * 2 * (usage[i] - 1.0/self.n_experts)
...
def run():
    ...
    for name, (X, y) in datasets.items():
        print(f"\n===== DATASET: {name} =====")
        model = V60MoE(input_dim=X.shape[1], hidden=16, output_dim=len(np.unique(y)), n_experts=5)
        n_epochs = 3
    ...
    with open("resultados_finais/v6_0_stable_moe.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nSaved -> resultados_finais/v6_0_stable_moe.json")
